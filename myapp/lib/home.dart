...
class _Screen4State extends State<Screen4> with TickerProviderStateMixin {
  ...
  final ValueNotifier<bool> _chatHasText = ValueNotifier<bool>(false);
  ...
  void initState() {
    ...
    _suggestionTimer = Timer.periodic(const Duration(seconds: 6), (_) {
    ...
    _chatController.addListener(() {
      ...
      if (hasText != _chatHasText.value) _chatHasText.value = hasText;
    });
  ...
  void _handleQuery(String question, String intent) {
    ...
    _setThinkingActive(true);
    Future.delayed(const Duration(milliseconds: 1800), () {
      if (!mounted) return;
      final resp =
          _responseMap[question] ??
          _ResponseData(type: 'text', question: question);
      setState(() {
        _overlayState = _OverlayState.response;
        _responseData = resp;
      });
      _setThinkingActive(false);
  ...
  void _setThinkingActive(bool active) {
    if (active) {
      if (!_thinkingCtrl.isAnimating) _thinkingCtrl.repeat();
      return;
    }
    if (_thinkingCtrl.isAnimating) _thinkingCtrl.stop();
    _thinkingCtrl.value = 0.0;
  }
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Positioned(left: 0, right: 0, bottom: 96, child: _buildChatWrap()),
  ...
  Widget _buildAuroraLayer() {
    ...
    final enabled =
        routeActive && !_homeCollapsed && _overlayState == _OverlayState.idle;
    _syncAuroraActive(enabled);
  ...
  void _setAuroraActive(bool enabled) {
    if (enabled) {
      if (!_aurora1Ctrl.isAnimating) {
        _aurora1Ctrl.repeat(reverse: true);
      }
      if (!_aurora2Ctrl.isAnimating) {
        _aurora2Ctrl.repeat(reverse: true);
      }
      if (!_aurora3Ctrl.isAnimating) {
        _aurora3Ctrl.repeat(reverse: true);
      }
      return;
    }
    if (_aurora1Ctrl.isAnimating) _aurora1Ctrl.stop();
    if (_aurora2Ctrl.isAnimating) _aurora2Ctrl.stop();
    if (_aurora3Ctrl.isAnimating) _aurora3Ctrl.stop();
  }

  void _syncAuroraActive(bool enabled) {
    if (_auroraActive == enabled) return;
    _auroraActive = enabled;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      _setAuroraActive(_auroraActive);
    });
  }
          ...
          builder: (context, constraints) {
            ...
            return Row(
              children: [
                ...
                if (!compact) ...[_buildMicButton(), const SizedBox(width: 6)],
                _AnimatedPressable(
                  liftY: -1.5,
                  scalePressed: 0.90,
                  onTap: () {
...
class _NavPillPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    bp.cubicTo(
      lx + tang,
      pillTop, // cp1: leaves foot horizontally
      cx - tang,
      peakY, // cp2: arrives at peak horizontally
      cx,
      peakY, // peak
    );
    bp.cubicTo(
      cx + tang,
      peakY, // cp1: leaves peak horizontally
      rx - tang,
      pillTop, // cp2: arrives at foot horizontally
      rx,
      pillTop, // right foot
    );
...
final _responseMap = <String, _ResponseData>{
  ...
  'Plan a 3-day Goa trip': _ResponseData(
    ...
    planSections: [
      _PlanSection('Day 1 — Arrival & North Goa', _accent, [
        '☀️ Arrive & check in',
        '🏖️ Baga Beach',
        '🍽️ Dinner at Thalassa',
        '🍹 Night — Tito\'s Lane',
      ]),
      _PlanSection('Day 2 — Culture & South Goa', _accentSecondary, [
        '🏛️ Old Goa churches',
        '🚗 Drive to Palolem',
        '🏄 Kayaking',
        '🌅 Sunset at Cabo de Rama',
      ]),
      _PlanSection('Day 3 — Relax & Depart', _accentTertiary, [
        '🧘 Morning yoga',
  ...
  'Pack for business travel': _ResponseData(
    ...
    planSections: [
      _PlanSection('👔 Clothing', _accent, [
        '2× formal shirts (navy, white)',
        '1× blazer (charcoal)',
        '2× trousers',
        '1× casual outfit',
      ]),
      _PlanSection('💼 Work Essentials', _accent, [
        'Laptop + charger + adapter',
        'Notebook + 2 pens',
        'Business cards',
        'Portable battery',
      ]),
      _PlanSection('🧴 Toiletries', _accentSecondary, [
        'Moisturiser, deodorant, perfume',
        'Toothbrush + paste',
        'Face wash + razor',
      ]),
    ],
  ...
  'Create a wedding checklist': _ResponseData(
    ...
    planSections: [
      _PlanSection('📅 6–12 Months Before', _accent, [
        'Set budget & guest list',
        'Book venue & caterer',
        'Book photographer',
        'Send save-the-dates',
      ]),
      _PlanSection('🎨 3–6 Months Before', _accentTertiary, [
        'Send invitations',
        'Finalise menu & cake',
        'Book hair & makeup',
        'Order wedding outfits',
      ]),
      _PlanSection('📋 1 Month Before', _accentSecondary, [
        'Confirm all vendor bookings',
        'Finalise seating',
        'Collect RSVPs',
      ]),
      _PlanSection('✅ Week Of', _accentTertiary, [
        'Final dress fitting',
        'Prepare wedding day kit',
        'Brief photographer & MC',
        'Rest & enjoy 🎉',
      ]),
    ],
  ),
...
class _EntryFadeSlide extends StatefulWidget {
  ...
  const _EntryFadeSlide({super.key, required this.child});
...
class _CardPressable extends StatefulWidget {
  ...
  const _CardPressable({required this.builder, this.onTap});
