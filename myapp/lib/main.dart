...
import 'package:myapp/theme/theme_storage.dart';
...
Future<void> main() async {
  ...
  final initialTheme = await ThemeStorage.loadTheme();
  final initialDarkMode = await ThemeStorage.loadMode();
  runApp(MyApp(initialTheme: initialTheme, initialDarkMode: initialDarkMode));
}
...
class MyApp extends StatelessWidget {
  final ProfileTheme initialTheme;
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(create: (context) => AuthUserController()),
...
class _MainNavigationShellState extends State<MainNavigationShell>
  ...
  bool _lensLaunchInProgress = false;
  ...
  void _handleNavTap(int idx) {
    if (_lensLaunchInProgress) return;
    if (idx == 2) {
      _openLensSheet();
      return;
    }
  ...
  // ── Lens sheet ─────────────────────────────────────────────────────────────
  void _openLensSheet() {
    if (_lensLaunchInProgress || _lensSheetOpen) return;
  ...
  Future<void> _launchLensCamera(LensEntryAction action) async {
    if (_lensLaunchInProgress) return;
    setState(() => _lensLaunchInProgress = true);
    HapticFeedback.mediumImpact();
    try {
      if (_lensSheetOpen) {
        await _lensSheetCtrl.animateBack(
          0.0,
          duration: _A.normal,
          curve: _A.ease,
        );
        if (mounted) {
          setState(() => _lensSheetOpen = false);
        }
      }
      if (!mounted) return;
      await Navigator.of(context).push(
        PageRouteBuilder<void>(
          transitionDuration: _A.normal,
          reverseTransitionDuration: _A.fast,
          pageBuilder: (context, animation, secondaryAnimation) {
            return FadeTransition(
              ...
              child: SlideTransition(
                ...
                child: LensCameraPage(action: action),
              ),
            );
          },
        ),
      ...
      if (mounted) {
        setState(() => _lensLaunchInProgress = false);
      }
    }
  }
              ...
              child: GestureDetector(
                ...
                child: Transform.translate(
                  ...
                  child: _LensSheetContent(
                    ...
                    launchInProgress: _lensLaunchInProgress,
                    onOptionTap: _launchLensCamera,
                  ),
                ),
              ),
            ),
...
class _LensSheetContent extends StatelessWidget {
  ...
  final bool launchInProgress;
  final ValueChanged<LensEntryAction> onOptionTap;

  const _LensSheetContent({
    required this.t,
    required this.onClose,
    required this.launchInProgress,
      ...
      child: Column(
        ...
        children: [
          ...
          _LensOption(
            ...
            onTap: () => onOptionTap(LensEntryAction.findSimilar),
          ),
          _LensOption(
            ...
            mutedColor: t.mutedText,
            enabled: !launchInProgress,
            onTap: () => onOptionTap(LensEntryAction.addToWardrobe),
          ),
          _LensOption(
            ...
            mutedColor: t.mutedText,
            enabled: !launchInProgress,
            onTap: () => onOptionTap(LensEntryAction.askAhvi),
          ),
        ],
      ),
    );
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: widget.enabled
              ? widget.panelColor
              : widget.panelColor.withValues(alpha: 0.55),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
...
class _HomePageHost extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const ClipRect(child: home.Screen4());
  }
}
